...
class DisplayTable<T> extends React.PureComponent<DisplayTableProps & T, {}> {
  render () {
    ...
    const removeInvalidElements = React.Children.toArray(children).filter(isValidElement);
    return (
      <table className={className}>
          ...
          <tr>
            {removeInvalidElements.map((tableCol:TableColElement) => {
              if(isValidElement(tableCol)){
                ...
                return <th className={className}>{headerLabel || startCase(name)}</th>
              }
            })}
          </tr>
        </thead>
        <tbody>
          {data.map((dataItem:Map<string, any>) => (
            <tr className={dataItem.has('disabled') && dataItem.get('disabled') ? 'disabled' : null}>
              {removeInvalidElements.map((tableCol:TableColElement) => {
                if(isValidElement(tableCol) && tableCol && tableCol.props){
                  const {name, transform, tdClassName, children: tableColChildren} = tableCol.props;
                  if(name && Iterable.isIterable(dataItem) && dataItem.has(name)){
                    if(isFunction(transform)){
                      return <td className={tdClassName}>{transform(dataItem.get(name), Object.assign({}, {dataItem}, this.props))}</td>
                    } else {
                      return <td className={tdClassName}>{dataItem.get(name)}</td>
                    }
                  } else if(React.Children.count(tableCol) === 1 && tableColChildren){
                    return <td className={tdClassName}>{React.cloneElement(tableColChildren, Object.assign({}, {dataItem}, this.props))}</td>
                  }
                }
                return <td></td>;
              })}
