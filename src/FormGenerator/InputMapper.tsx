 ...
 */
class InputMapper extends React.Component<(InputMapperProps & InputUnionProps) & WithPropsProps, {}>{
  ...
  getContext({
    ...
    const renameAttributes = attributes.mapEntries(([key, value]) => {
      if (key === 'title') {
        ...
        return [key, (window as any).client.getIn([resourceName, resourceName + "_Query", 'api'])];
      } else if (Iterable.isIterable(value) && value.has('x-blue-ApiResourceReference')) {
        const resourceName:string = value.getIn(['x-blue-ApiResourceReference', 'resourceName']);
        const resourcePath = resourceName.split('/');
        if(size(resourcePath) > 1){
          return [key, value.setIn(['x-blue-ApiResourceReference', 'api'], (window as any).client.getIn([resourcePath[0], `${resourcePath[0]}_${resourcePath[1]}`, 'api']))]
        } else {
          return [key, value.setIn(['x-blue-ApiResourceReference', 'api'], (window as any).client.getIn([resourceName, resourceName + "_Query", 'api']))]
        }
      }
      return [key, value];
    });
