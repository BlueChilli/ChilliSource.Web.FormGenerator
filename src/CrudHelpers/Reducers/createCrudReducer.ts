...
const convertIdKeyedMap = (data = Map<string, Map<string, any>>({})) => {
  return data.map((value: Map<string, any>) => {
    return List([value.get("id"), value]);
  });
};

const getPaginatedResolution = (state: State, action: CrudAction) => {
  const { payload } = action;
  const { stateName } = action.meta;
  return state.updateIn(
    [stateName],
    OrderedMap(),
    (currentState: OrderedMap<string, any>) => {
      ...
      const currentData: OrderedMap<string, any> = mergedPageData.get(
        "data",
        OrderedMap({})
      );
      const updatedData: OrderedMap<string, any> = currentData.merge(
        convertIdKeyedMap(payload.get("data"))
      );
...
const filterPaginatedResolution = (state: State, action: CrudAction) => {
  const { payload } = action;
  const { stateName } = action.meta;
  const keyedPayload = payload.update("data", Map(), data =>
    Map(convertIdKeyedMap(data))
  );
  return state.set(stateName, keyedPayload);
};

const getSingleResolution = (state: State, action: CrudAction) => {
  const { payload } = action;
  const { stateName } = action.meta;
  return state.mergeIn(
    [stateName, "data"],
    Map({ [payload.get("id")]: payload })
  );
};

const getResolution = (state: State, action: CrudAction) => {
  const { payload } = action;
  const { stateName } = action.meta;
...
export default (stateName: string, extend: ExtendObject = {}) => {
  return createReducer<State, any>(initialState, {
    ...
    [`SET_${stateName}_EDIT_LIST_ITEM`]: (
      state,
      action: SetEditListItemAction
    ) => {
      const { payload } = action;
      const { stateName } = action.meta;
      return state.set(
